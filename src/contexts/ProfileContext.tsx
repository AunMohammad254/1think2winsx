  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Use session data as fallback - no mock data
        if (session?.user) {
  ...
  const updateProfile = async (data: { name: string; email: string }) => {
    try {
      ...
      // Update session data if available
       if (session?.user) {
         // Note: Session updates are handled automatically by NextAuth
         // The navbar will reflect changes through the profile context
       }
